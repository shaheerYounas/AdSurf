...
| DELETE | `/v1/workspaces/{workspace_id}/recommendations/{recommendation_id}` | Delete one recommendation from the AdSurf review queue and remove related decision rows. This is queue cleanup only. |
| POST | `/v1/workspaces/{workspace_id}/recommendations/bulk-delete` | Delete selected recommendations from the AdSurf review queue. Request body is `{"recommendation_ids": ["uuid"]}`. |
...
Sponsored Products Search Term Report columns are preserved as source evidence when uploaded through this workflow. Product-profile fields still map only to the product contract, but report headers such as campaign, ad group, targeting, search term, impressions, clicks, spend, sales, ACOS, ROAS, orders, and units are returned in `detected_columns` as `source_evidence` and stored in each row's `raw_row_json`. If a report row lacks explicit product name, ASIN, or SKU columns, the preview derives a reviewable product name from campaign/ad group/targeting and a stable synthetic SKU from row evidence; valid rows still require human confirmation before product profiles are created.
...
Validation follows the product profile contract: product name is required unless it can be derived from Sponsored Products report campaign/ad group/targeting evidence; ASIN is normalized to uppercase and must be 10 alphanumeric characters when present; ASIN or SKU is required for import identity, with report uploads allowed to derive a stable synthetic SKU from source evidence; target ACOS accepts `30`, `30%`, `0.30`, and `0.3` and stores a decimal percentage; budget and bid parse common currency text but must be positive with at most 4 decimal places; marketplace/currency must match supported Amazon marketplace defaults. Missing target ACOS uses the user-supplied import default when provided, otherwise the product schema default.
