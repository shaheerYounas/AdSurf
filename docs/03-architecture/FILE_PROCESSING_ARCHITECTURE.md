...
- Safeguards classify import health messages as info, warning, error, or critical for report-type uncertainty, missing columns, hidden header spaces, known Amazon column-name aliases, currency or marketplace mismatch, mixed attribution windows, mixed date ranges, invalid date ranges, non-numeric metric values, ambiguous or invalid percentage values, metric formula mismatches, negative metrics, ASIN-like search terms, mixed ASIN/keyword search-term rows, auto/product-targeting contexts, missing match type, exact duplicate rows, duplicate search-term contexts, orders/units confusion, other-SKU sales dominance, low-data rows, import-level insufficient optimization evidence, and margin-risk ACOS. Known Amazon aliases are info. Blank ACOS/ROAS when sales are zero is normal and is not a warning.
