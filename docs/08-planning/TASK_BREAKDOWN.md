# Task Breakdown

| Epic | Tasks |
| --- | --- |
| Foundation | Repo scaffold, docs, env example, ignore rules. |
| Auth and tenancy | Supabase Auth, tenant tables, roles, RLS policies. |
| Product profiles | CRUD, defaults, validation, audit events. |
| File processing | Uploads, storage, parser, mapping, row issues. |
| Scoring | Relevance score rules, rejection, explanations, tests. |
| Campaign planning | Hero, grouping, match types, negatives, plan approval. |
| Bulk exports | Sheet rows, validation, storage, approval, download. |
| Monitoring | Report imports, metric calculations, 14-day timeline. |
| Recommendations | Rule triggers, approval queue, audit events. |
| UI | Pages, tables, forms, charts, empty states. |
| Quality | Unit, integration, E2E, CI/CD, observability. |

## Definition Of Done
Task is done only when implementation, tests, docs, and approval/audit checks are complete.

