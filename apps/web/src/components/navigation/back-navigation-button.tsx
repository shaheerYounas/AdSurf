...
import { getBackNavigationTarget, isInternalNavigationPath, normalizePathname } from "@/lib/navigation/back-navigation";
...
export function BackNavigationButton() {
  ...
  const targetLabel = previousPath ? "Previous page" : `Back to ${fallbackTarget.label}`;

  return (
    <nav className="mb-5 flex items-center justify-between gap-3" aria-label="Page history">
      <Link
      ...
      >
        ...
        {!previousPath ? <span className="hidden text-xs font-medium text-slate-500 dark:text-slate-400 sm:inline">Fallback: {fallbackTarget.label}</span> : null}
      </Link>
    </nav>
  );
}
